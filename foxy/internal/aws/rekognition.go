...
func DetectFaces(sourceConfig db.SourceConfig, sid string, key string, sourceImage *vips.ImageRef, skipCache bool) (*metadata.Metadata, error) {
	...
	if sourceConfig.RekognitionKey == nil || sourceConfig.RekognitionSecret == nil {
		return nil, errors.New("rekognition key and secret not set")
	}

	creds := credentials.NewStaticCredentials(
		*sourceConfig.RekognitionKey,
		*sourceConfig.RekognitionSecret,
		"",
	)

	sess := session.Must(session.NewSession(&aws.Config{
		Region:      sourceConfig.RekognitionRegion,
		Credentials: creds,
	}))
	...
	if sourceConfig.SourceType != "s3" || sourceImage.Width() > 2560 || sourceImage.Height() > 2560 {
